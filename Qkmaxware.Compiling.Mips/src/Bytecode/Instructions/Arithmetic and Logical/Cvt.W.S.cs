...
/// <summary>
/// Convert single to integer (MIPS cvt.w.s)
/// </summary>
public class CvtWS : FloatingPointEncodedInstruction, IAssemblyInstruction {
    ...
    /// </summary>
    /// <returns>description</returns>
    public string InstructionDescription() => "Convert the single value in $arg into an integer and store it in $dest.";
    ...
    public override void Invoke(Cpu cpu, Fpu fpu, IMemory memory, SimulatorIO io) {
        ...
        fpu.Registers[this.Destination].WriteInt32((int)(lhs));
    }

    public override uint Encode32() {
        //   OOOOOOCC CCCTTTTT DDDDDIII IIIIIIII
        return new WordEncoder()
            .Encode(0x11U, 26..32).Encode(0x10U, 21..26).Encode(0, 16..21).Encode((uint)Source, 11..16).Encode((uint)Destination, 6..11).Encode(24, 0..6)
            .Encoded;
    }
    ...
    public static bool TryDecodeAssembly(Assembly.IdentifierToken opcode, List<Mips.Assembly.Token> args, out IAssemblyInstruction? decoded) {
        ...
        if (!IsAssemblyFormatDestArg<CvtWS, Assembly.RegisterToken, Assembly.RegisterToken>(opcode, args, out dest, out arg)) {
            decoded = null;
            return false;
        }
